...
public class BodyTechPage extends Page {
	...
	public BodyTechPage(int pageId, String userMail, int regionId,
	...
	public static BodyTechPage addNewBodyTechPage(String userMail,
		...
		String sql = "insert into kaputo.BodyTechPage (`UserMail` , `RegionRegionID` , `Address` , `Latitude` ,`Longitude` , `Title` , `Description` ,`StartTime`,`EndTime`,`Phone`  ) values (?  ,? ,? , ? ,? , ?  ,? ,? , ? ,? );";
		...
		if (rs.next()) {
			...
			BodyTechPage bodyTechPage = new BodyTechPage(pageId, userMail,
					regionId, lat, lon, address, title, description, startTime,
					endTime, phone);
